...
function contentMCMOT(date, sortie, clip, segments, inputDir, fps) {
...
let countImg = 0
let countXml = 0
async function convertTxtToMCMOT(inputDir, outDir, fps, digitFileName, date, sortie, clip, mode) {
    let fileData = {};
    ...
    clipFolderFiles.forEach(drone => {
        ...
        if (mode === '4') {
            ...
            if(droneImgFiles.length > 0) {
                ...
                if (mode !== '5') {
                    const processFiles = async () => {
                        const [contentTargetBox, contentTargetMain, contentTargetPos, contentTargetBoxPPK] = contentMCMOT(date, sortie, clip, fileData, inputDir, fps)
                        await Promise.all([
...
async function convertMCMOT(inputDir, outDir, fps, digitFileName, date, sortie, clip, mode) {
    try {
        await convertTxtToMCMOT(inputDir, outDir, fps, digitFileName, date, sortie, clip, mode)
    } catch (error) {
        console.error(error)
    }
}
