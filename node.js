...
const { drawText, drawBoundingBox, handleImageDET, handleImageMOT, handleImageMoving} = require('./images');
...
function convertTxtToDet (date, droneName, clipName, fileInput, sortie, index, unplanned = true) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // const fileName = file.split('_').slice(0, 5).join('_');

      // const fileURL = path.join(inputClipDir, 'xml', fileName+'.xml');
      ...
      //const fileContent = convertXML2JSON(fileURL);
    ...
    try {
      ...
      if(clipsSplitFiles.length > 0) {
        for(const file of clipsSplitFiles) {
          ...
          await convertToFrames(videoUrl, imagesPath, 50, fileName);
      ...
      const outputImagePath = path.join(outputDir, 'images1');
      if (!fs.existsSync(outDir + outputImagePath)) {
        createDirectory(outputImagePath)
      }

      for (let index = 500; index < 650; index = index + 5) {
...
function convertToFrames(inputVideo, outputFramesDir, fps, fileName, startFrame, endFrame) {
  return new Promise((resolve, reject) => {
